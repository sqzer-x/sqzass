...
| 키 | 기본값 | |
|---|---|---|
| `enabled` | `true` | 언어별 `search-<lang>.json`을 만듭니다. |

색인은 모든 페이지의 본문을 담아 콘텐츠와 함께 커집니다 — 검색 UI가 없는
사이트가 그 값을 치를 이유가 없습니다. 끄면 색인이 만들어지지 않고, 링크가
가리킬 색인 URL도 존재하지 않게 됩니다. [검색](@/features/search.md)을
참고하세요.
